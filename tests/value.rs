...
use rollback_mlua::{Error, LightUserData, Lua, MultiValue, Result, StdLib, Value};
...
fn test_value_conversions() -> Result<()> {
    ...
    {
        assert_eq!(Value::Integer(rollback_mlua::Integer::MAX).as_i32(), None);
        assert_eq!(Value::Integer(rollback_mlua::Integer::MAX).as_u32(), None);
    }
